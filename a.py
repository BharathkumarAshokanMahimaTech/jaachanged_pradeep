...
def get_work(db:Session,user_id:int):
    ...
    for row in db_res:
        ...
        data['created_on'] = row.created_on.strftime("%d-%m-%Y %H:%M:%S")
...
def User_Wise_Day_Wise_Part_1(db: Session, picked_date: str, to_date: str):
    ...
    for row in db_res:
        ...
        for row2 in db_res2:
            print(db_res2,'hfdgjhofidghiogdhfgnd')
        ...
        if row.work_status == "Reallocated":
            ...
            a = work_hour_hours_diff - re_hour_diff
        if row.work_status == "Completed":
            a = work_hour_hours_diff - end_hour_diff
        else:
            b = work_hour_hours_diff - end_hour_diff
        ...
        if row.work_status == "Completed":
            ...
            data["completed"] = (datetime.strptime(row.completed_time, date_time_formate_string) - datetime.strptime(row.working_time, date_time_formate_string)) - (call_hour_diff + break_hour_diff + meet_hour_diff + hold_hour_diff)
        else:
            data["completed"] = timedelta(hours=0)
            data["in_progress"] = (a + b) - (call_hour_diff + break_hour_diff + meet_hour_diff + hold_hour_diff + end_hour_diff)
...
#--------------------------------------------------------------------------------------------------------------------------------------
        ...
        if row.type_of_activity == "CHARGABLE":
            data["chargable_time"] = data["in_progress"]
        else:
